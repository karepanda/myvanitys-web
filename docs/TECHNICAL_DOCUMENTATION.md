...
- **Key Features:**
  ...
  - Dockerized deployment with a multi-stage build (Vite → Nginx)
...
- [ ] Node.js 18.x (see `engines` in [`package.json`](../package.json#L19))
...
> **Note:** There is no `.env.example` file in this repository. The files [`.env.development`](../.env.development) and [`.env.production`](../.env.production) are committed directly. The variable names above are extracted from those files; values are deliberately omitted from the production file.
...
- **Strategy Pattern (Auth):** [`authService.handleAuthentication()`](../src/services/auth/authService.js#L17) inspects the OAuth2 `state` parameter to dispatch to either `loginService` or `registerService`, allowing the same Google redirect to serve two flows.
...
> This is a **frontend-only** repository. The backend is a separate Spring Boot project (`../myvanitys-api`) referenced in [`docker-compose.yml`](../docker-compose.yml#L27). All endpoints below are **consumed** by this frontend — they are defined in the service layer files under [`src/services/`](../src/services/). The base URL is `${VITE_API_URL}`.

| Method | Path                                       | Description                                                   | Auth Required | Source File                                                                                 |
|--------|--------------------------------------------|---------------------------------------------------------------|---------------|---------------------------------------------------------------------------------------------|
| POST   | `/auth/google`                             | Google OAuth2 login — exchanges auth code for JWT token       | No            | [`loginService.js`](../src/services/auth/loginService.js#L24)                               |
| POST   | `/auth/register`                           | Google OAuth2 registration — creates new user account         | No            | [`registerService.js`](../src/services/auth/registerService.js#L29)                         |
| GET    | `/users/profile`                           | Fetch current user's profile data                             | Yes           | [`useFetchUserData.js`](../src/hooks/useFetchUserData.js#L18)                               |
| GET    | `/products`                                | List all products (with collection status when authenticated) | Optional      | [`readProductService.js`](../src/services/product/operations/readProductService.js#L12)     |
| GET    | `/products?page={page}&size={size}`        | Paginated product listing                                     | Optional      | [`readProductService.js`](../src/services/product/operations/readProductService.js#L155)    |
| GET    | `/products/{productId}`                    | Get a single product by ID                                    | Yes           | [`readProductService.js`](../src/services/product/operations/readProductService.js#L23)     |
| GET    | `/products/featured?limit={limit}`         | Get featured products                                         | Yes           | [`readProductService.js`](../src/services/product/operations/readProductService.js#L71)     |
| GET    | `/products/categories`                     | List all product categories                                   | Yes           | [`readProductService.js`](../src/services/product/operations/readProductService.js#L169)    |
| GET    | `/products/search?query={query}`           | Full-text product search (min 2 chars)                        | Yes           | [`searchProductService.js`](../src/services/product/operations/searchProductService.js#L40) |
| GET    | `/products/category/{categoryId}`          | Search products by category                                   | Yes           | [`searchProductService.js`](../src/services/product/operations/searchProductService.js#L73) |
| GET    | `/users/{userId}/products`                 | Get all products owned by a specific user                     | Yes           | [`readProductService.js`](../src/services/product/operations/readProductService.js#L45)     |
| POST   | `/products`                                | Create a new product (name, brand, categoryId, colorHex)      | Yes           | [`createProductService.js`](../src/services/product/operations/createProductService.js#L22) |
| POST   | `/products/{productId}/add-to-vanity`      | Add an existing product to the current user's vanity          | Yes           | [`readProductService.js`](../src/services/product/operations/readProductService.js#L131)    |
| PUT    | `/products/{productId}`                    | Update product details                                        | Yes           | [`updateProductService.js`](../src/services/product/operations/updateProductService.js#L14) |
| PUT    | `/products/{productId}/status`             | Toggle product active/inactive status                         | Yes           | [`updateProductService.js`](../src/services/product/operations/updateProductService.js#L32) |
| PUT    | `/products/{productId}/price`              | Update product price                                          | Yes           | [`updateProductService.js`](../src/services/product/operations/updateProductService.js#L48) |
| DELETE | `/products/{productId}`                    | Delete a product                                              | Yes           | [`deleteProductService.js`](../src/services/product/operations/deleteProductService.js#L13) |
| POST   | `/products/{productId}/reviews`            | Add a review (rating 1-5 + comment)                           | Yes           | [`reviewService.js`](../src/services/product/operations/reviewService.js#L31)               |
| GET    | `/products/{productId}/reviews`            | Get reviews for a product (optional page/limit/sortBy)        | No            | [`reviewService.js`](../src/services/product/operations/reviewService.js#L57)               |
| PUT    | `/products/{productId}/reviews/{reviewId}` | Update a review (rating or comment)                           | Yes           | [`reviewService.js`](../src/services/product/operations/reviewService.js#L92)               |
| DELETE | `/products/{productId}/reviews/{reviewId}` | Delete a review                                               | Yes           | [`reviewService.js`](../src/services/product/operations/reviewService.js#L109)              |
| GET    | `/user/reviews`                            | Get current user's reviews (optional page/limit)              | Yes           | [`reviewService.js`](../src/services/product/operations/reviewService.js#L134)              |
...
- **Key Rules / Plugins:**
  ...
  - Browser globals enabled (no Node.js globals in a source)

### IDE Integration

- **Recommended IDE:** VS Code (based on a workspace file at [`src/components/Notification/myvanitys-web.code-workspace`](../src/components/Notification/myvanitys-web.code-workspace))
...
4. **Auth token in localStorage** — The JWT token and user object are persisted under the key `vanitys_auth`. The context autoloads this on mount via `loadSavedAuth()`, using locally decoded expiry (with a 30-day fallback) only to decide whether to restore the session.
5. **Bearer token prefix normalization** — [`createProductService.js`](../src/services/product/operations/createProductService.js#L7) auto-prepends `"Bearer "` if the token is missing it, while the adapter and other services expect the token already prefixed.

> **Security note — client-side JWT inspection is UX logic, not a security check.**
> The frontend decodes the JWT `exp` claim ([`getJwtExpiration()`](../src/utils/jwt.js#L7)) only to estimate when to stop restoring or using a locally cached session. It does **not** verify the JWT signature, issuer, or any other claim, and must not be treated as authentication or authorization. When `exp` cannot be read, the frontend applies a `Date.now() + 30 days` local fallback — in the OAuth callback ([`AuthCallbackHandler.jsx`](../src/components/Auth/AuthCallbackHandler.jsx)) and in [`context/index.jsx`](../src/context/index.jsx) `updateAuthData()` — so that 30-day value is only a local fallback, not a promise about the server-enforced token lifetime. The backend is authoritative: on every protected request, `JwtAuthenticationFilter` calls `TokenService.extractUserId()`, which reaches `JwtTokenGeneratorAdapter.parseToken()` to verify the signed token and its expiry, and rejects invalid or expired tokens with `401 Unauthorized`. A frontend route guard (e.g. [`ProtectedRoute`](../src/components/ProtectedRoute/ProtectedRoute.jsx)) or a client-side JWT library cannot replace backend verification — signing keys must never be shipped to the browser.
...
- **Nginx Configuration:** ([`nginx.conf.template`](../nginx.conf.template))
  ...
  - Serves static files with a 1-year cache for JS/CSS/images.
  ...
  - Gzip compression is enabled for text and JSON content types.
...
docker compose up         # Full stack startup (PostgresSQL + API + Frontend)
...
2. [`authService.initiateGoogleAuth(mode)`](../src/services/auth/authService.js#L8) calls [`googleAuthAdapter.redirectToGoogleOAuth()`](../src/services/auth/googleAuthAdapter.js#L3), which builds and navigates to: `https://accounts.google.com/o/oauth2/v2/auth?client_id=...&redirect_uri=...&response_type=code&scope=...&state=login|register`.
3. After consent, Google redirects to `/callback?code=...&state=...`.
4. [`AuthCallbackHandler`](../src/components/Auth/AuthCallbackHandler.jsx) calls [`authService.handleAuthentication()`](../src/services/auth/authService.js#L17) which parses the code and state.
...
6. On success, [`updateAuthData()`](../src/context/index.jsx#L118) saves to React context and `localStorage.vanitys_auth`, then navigates to `/dashboard`.
